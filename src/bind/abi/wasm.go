...
func NewWASMDeployTx(options TxOptions, wasm []byte) (*types.Transaction, error) {
	...
	return assembleUnsignedTx(options, wasm, nil, "", new(big.Int).Add(base.GasLimit, contract.GasLimit), new(big.Int).Set(base.GasPrice), nil), nil
}
...
func NewWASMCallTx(options TxOptions, contractAddress string, callData []byte) (*types.Transaction, error) {
	...
	return assembleUnsignedTx(options, nil, callData, contractAddress, new(big.Int).Add(base.GasLimit, contract.GasLimit), new(big.Int).Set(base.GasPrice), nil), nil
}

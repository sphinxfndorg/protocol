...
// go/src/bind/legacy.go
//
// Legacy same-box devnet harness — split out of nodes.go so that file can
// stay focused on the production StartNode path (see nodes.go's own banner
// comment: "Production node startup (previously in cli/utils/nodes.go)").
//
// Everything below predates StartNode and is reachable today only via
// cli.go's legacyExecute() -> bind.RunMultipleNodesInternal(). It hardcodes
// a 3-node same-box cluster on fixed ports (32307+) and does its own
// LevelDB/SPHINCS/consensus wiring independently of the modern key-exchange,
// PBFT leader-election, and peer-discovery logic in nodes.go/helpers.go.
// Kept for backward compatibility with the legacy CLI entrypoint; new work
// should go through StartNode.
package bind
...
// StartSingleNodeInternal starts a single node with the given configuration

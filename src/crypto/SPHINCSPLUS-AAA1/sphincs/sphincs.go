...
type SPHINCS_PK struct {
	PKseed []byte
	PKroot []byte
	FullPK []byte // Combined public key (PKseed + PKroot)
}
...
// Wrapper functions for SPHINCS_AAA1
func SPHINCS_AAA1_Keygen() (*SPHINCS_SK, *SPHINCS_PK) {
	params := parameters.SPHINCS_AAA1()
	return Spx_keygen_with_params(params)
}

func SPHINCS_AAA1_Sign(M []byte, SK *SPHINCS_SK) *SPHINCS_SIG {
	params := parameters.SPHINCS_AAA1()
	return Spx_sign_with_params(params, M, SK)
}

func SPHINCS_AAA1_Verify(M []byte, SIG *SPHINCS_SIG, PK *SPHINCS_PK) bool {
	params := parameters.SPHINCS_AAA1()
	return Spx_verify_with_params(params, M, SIG, PK)
}

// Helper functions
func Spx_keygen_with_params(params *parameters.Parameters) (*SPHINCS_SK, *SPHINCS_PK) {
	...
	pk.PKseed = PKseed
	pk.PKroot = PKroot
	pk.FullPK = append(pk.PKseed, pk.PKroot...) // Combine to 32 bytes

	return sk, pk
}

func Spx_sign_with_params(params *parameters.Parameters, M []byte, SK *SPHINCS_SK) *SPHINCS_SIG {
...
func Spx_verify_with_params(params *parameters.Parameters, M []byte, SIG *SPHINCS_SIG, PK *SPHINCS_PK) bool {
	adrs := new(address.ADRS)
	R := SIG.GetR()

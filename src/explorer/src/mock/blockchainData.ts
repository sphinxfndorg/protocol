  ...
  for (let i = 15; i >= 0; i--) {
    ...
    for (let t = 0; t < txCount; t++) {
      ...
      const tx: Transaction = {
        ...
        // Full SPHINCS+ auth bundle so the mock chain renders the same
        // signature panels the live node serves. Lengths match what
        // HasFullAuthBundle() requires server-side (hash/merkle/commitment/
        // proof 32 bytes, auth timestamp 8, auth nonce 16).
        signatureHash: genHex(64),
        commitment: genHex(64),
        authTimestamp: genHex(16),
        authNonce: genHex(32),
        gasUsed: 85000,
        burnedThisTxSpx: '0',
        burnedThisTxNspx: '0',
